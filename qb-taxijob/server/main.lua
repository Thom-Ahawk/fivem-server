...
-- 🚖 GARAGE TAXI (FLOTTE PARTAGÉE)

QBCore.Functions.CreateCallback('taxi:getVehicles', function(source, cb)
    local vehicles = exports.oxmysql:query(
...
-- 🔁 SORTIE VEHICULE
RegisterNetEvent('taxi:updateState', function(plate, state)
    MySQL.update(
        'UPDATE player_vehicles SET state = ? WHERE plate = ?',
        { state, plate }
    )
end)

-- 💾 RANGEMENT VEHICULE
RegisterNetEvent('taxi:updateVehicle', function(plate, fuel, engine, body)
    MySQL.update(
        'UPDATE player_vehicles SET fuel = ?, engine = ?, body = ?, state = 1 WHERE plate = ?',
        { fuel, engine, body, plate }
    )
end)
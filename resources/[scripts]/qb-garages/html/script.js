...
function populateVehicleList(garageLabel, vehicles) {
    ...
    vehicles.forEach((v) => {
        ...
        if (v.balance && v.balance > 0) {
            financeInfo.textContent = "Restant: $" + v.balance.toFixed(0);
        } else {
            financeInfo.textContent = "Payé";
        }
    ...
    status = "Fourrière";
        ...
        ["fuel", "engine", "body"].forEach((statLabel) => {
            ...
            if (statLabel === "fuel") label.textContent = "Carburant";
if (statLabel === "engine") label.textContent = "Moteur";
if (statLabel === "body") label.textContent = "Carrosserie";

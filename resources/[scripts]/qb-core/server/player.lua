                    ...
                    if job and job.onduty then
                        ...
                        if payment > 0 then
                            ...
                            if job.name == "police" then
                                ...
                                local societyMoney = result and result.amount or 0

                                if societyMoney >= payment then
                                    MySQL.update('UPDATE management_funds SET amount = amount - ? WHERE job_name = ?', {payment, 'police'})
                                    Player.Functions.AddMoney('bank', payment)
                                    TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "💰 Salaire police payé par la société", "success")
                                else
                                    TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "❌ Budget police insuffisant", "error")
                                end

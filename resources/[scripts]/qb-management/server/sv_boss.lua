...
RegisterNetEvent('police:getMoneyMenu', function()
    local src = source

    local result = MySQL.single.await('SELECT amount FROM management_funds WHERE job_name = ?', {'police'})
    local amount = result and result.amount or 0

    TriggerClientEvent('police:showBudgetMenu', src, amount)
end)

RegisterNetEvent('police:withdrawMoney', function(amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    local result = MySQL.single.await('SELECT amount FROM management_funds WHERE job_name = ?', {'police'})
    local current = result and result.amount or 0

    if current >= amount then
        MySQL.update('UPDATE management_funds SET amount = amount - ? WHERE job_name = ?', {amount, 'police'})
        Player.Functions.AddMoney('bank', amount)
        TriggerClientEvent('QBCore:Notify', src, "💸 Retiré : $" .. amount, "success")
    else
        TriggerClientEvent('QBCore:Notify', src, "❌ Pas assez d'argent société", "error")
    end
end)

RegisterNetEvent('police:depositMoney', function(amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if Player.Functions.RemoveMoney('bank', amount) then
        MySQL.update('UPDATE management_funds SET amount = amount + ? WHERE job_name = ?', {amount, 'police'})
        TriggerClientEvent('QBCore:Notify', src, "💵 Déposé : $" .. amount, "success")
    else

...
{
    header = "💰 Voir budget",
    txt = "Voir l'argent de la société",
    icon = 'fa-solid fa-dollar-sign',
    params = {
        event = "police:openBudgetMenu"
    }
},
...
RegisterCommand("boss", function()
    ...
    if PlayerData.job.name ~= "police" then
        QBCore.Functions.Notify("Tu n'es pas police", "error")
        return
    end

    if not PlayerData.job.isboss then
        QBCore.Functions.Notify("Tu n'es pas le boss", "error")
        return
    end
...
RegisterNetEvent('police:withdraw', function()
    TriggerServerEvent('police:withdrawMoney', 10000)
end)

RegisterNetEvent('police:deposit', function()
    TriggerServerEvent('police:depositMoney', 10000)
end)

RegisterNetEvent('police:openBudgetMenu', function()
    TriggerServerEvent('police:getMoneyMenu')
end)

RegisterNetEvent('police:showBudgetMenu', function(amount)
    local menu = {
        {
            header = "💰 Budget Police",
            txt = "Argent disponible : $" .. amount,
            isMenuHeader = true
        },
        {
            header = "💵 Déposer 10 000$",
            txt = "Ajouter de l'argent société",
            params = {
                event = "police:deposit"
            }
        },
        {
            header = "💸 Retirer 10 000$",
            txt = "Retirer de l'argent société",
            params = {
                event = "police:withdraw"
            }
        },
        {
            header = "⬅ Retour",
